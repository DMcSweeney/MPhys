...
class LocalNet(BaseNet):
    ...
    def __init__(self, ddf_levels=None, **kwargs):
        ...
        self.ddf_levels = [0, 1, 2] if ddf_levels is None else ddf_levels
        ...
        hm = [layer.conv3_block(h1, nc[1], nc[2], name='local_deep_2')]
        ...
        hm += [layer.upsample_resnet_block(hm[0], hc1, nc[2], nc[1],
                                           name='local_up_1')] if min_level < 2 else []
        hm += [layer.upsample_resnet_block(hm[1], hc0, nc[1], nc[0],
                                           name='local_up_0')] if min_level < 1 else []

        self.ddf = tf.reduce_sum(tf.stack([layer.ddf_summand(hm[2-idx], nc[idx], self.image_size, name='sum_%d' % idx)
