...
def normalise(input):
    ...
    maxval = np.amax(input, axis=(1, 2, 3))
    minval = np.amax(input, axis=(1, 2, 3))
    print("maxi shape:", maxval.shape)
    median_max = np.median(maxval, axis=0)
    print("med_maxi shape:", median_max.shape)
    median_min = np.median(minval, axis=0)
